def pyQEDA(filename=None, Generate_random_diffs = False, N_training_data = 1, UseInputUgMasterList = False,\
				InputUg = None, InputUgdoFlip = False, scaling = 1, ReturnOutputUg = False, mode ='LARBED_SIM'):
	# 1. Ar + Master list
	...
	for training_data in range(N_training_data):
		...
		if mode == 'LARBED_SIM':
			...
			if UseInputUgMasterList:
				...
				thickness = InputUgRaw[-1]* scaling * 1000 #this *1000 is scaling back up to the correct value,the thickness, as found by the NN or GP. It is important to have the variables in the same range.
				...
				if InputUgdoFlip:
					print("do flip")
					beams = np.asarray(np.loadtxt("/Users/chenjie/pyQEDA/pyQEDA_simulations/" + fileName + "/" + fileName + "_experimental_data/_BeamList.txt",skiprows=1, usecols= range(1, 4)  ), dtype=int )               
					[UgList, potMapSim] = pqp.displayPotentialFromUg(folder=None, potSize=([256,256]), Ncells=([2,2]), showImag=True, doFlip=True, thresh = 0.01, beta = 0.9, InputUg=np.ravel([InputUg.real,InputUg.imag],'F'), InputBeams = beams, returnComplexUg=True, scaling=1)
	...
	if ReturnOutputUg:
		...
		return np.asarray(diff_pattern[0])
